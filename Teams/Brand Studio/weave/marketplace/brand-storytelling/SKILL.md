...
source: https://skillsmp.com/creators/refoundai/lenny-skills/skills-brand-storytelling
source_repo: https://github.com/RefoundAI/lenny-skills/tree/main/skills/brand-storytelling
author: RefoundAI (lenny-skills — insights from Lenny's Podcast guests)
copied_verbatim: true
fulfills_catalog_entry: storytelling-frameworks (VYON_Skills_Catalog_Full_v2.html, weave/Brand Studio)
...
<!--
  Unmodified copy of "brand-storytelling" (RefoundAI/lenny-skills, 1.1k★ — same repo family
  as marcus's setting-okrs-goals source), fulfilling the catalog's storytelling-frameworks
  slot. Chosen over generic hero's-journey/Pixar-spine writeups because it distills the same
  frameworks THROUGH 30 named practitioners with tactical advice per insight: Pixar spine
  (Moesta), hero's-journey casting (Petra Wille), customer-as-hero/Obi-Wan (Maples),
  five-second moment + stakes toolkit (Matthew Dicks), movement framing + rallying cry
  (Raskin), memification (Yamashata), dinner-test repeatability (Meservey), start-in-the-action
  (Merci Grace), problems-beat-successes (Feifer).
  ROLE IN THIS SYSTEM: this is weave's CRAFT layer — how any single chapter hooks, lands its
  moment, and gets retold. The ARCHITECTURE layer (which chapter it is, whether it may exist,
  continuity) is weave's custom brand-story-arcs skill, whose checks cite this craft.
  All chapters approved by brand-story-arcs get built with this skill's techniques, worded by
  lena, and gated by spark.
-->
